/**
 * UPTICK MT5 — where the candles come from.
 *
...
function fetchCandlesBatch(symbols, granularity = 300, count = 250, timeoutMs = 15000) {
  return new Promise((resolve) => {
    ...
    if (typeof WebSocket !== "function") {
      console.error("[uptick mt5] no WebSocket in this runtime — needs Node 22 or newer");
      return resolve(out);
    }

    let ws;
    try { ws = new WebSocket(WS_URL); }
    catch (e) { console.error("[uptick mt5] could not open the Deriv socket:", e); return resolve(out); }
