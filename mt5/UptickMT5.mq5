//+------------------------------------------------------------------+
//|                                                   UptickMT5.mq5   |
//|   Uptick — Deriv MT5 automation (custody-free)                    |
//|                                                                   |
//|   The AI/strategy runs in Uptick's cloud. This EA polls the       |
//|   signal feed and executes on YOUR terminal, on YOUR account —    |
//|   Uptick never sees a password. Put it on a VPS (Deriv/MT5        |
...
//|         https://legendary-palm-tree-tawny.vercel.app              |
//+------------------------------------------------------------------+
#property copyright "Uptick"
#property link      "https://legendary-palm-tree-tawny.vercel.app"
...
input long        InpMagic          = 99090001;   // Magic number (Uptick trades only)
...
string  g_base = "https://legendary-palm-tree-tawny.vercel.app/api/mt5/signals";
CTrade  g_trade;
...
int OnInit()
  {
   ...
   g_maxOpenRisk = GVget("up_capmax"+AcctSuffix(), 0);
   g_corrCap     = GVget("up_capcorr"+AcctSuffix(), 0);
   g_dailyLoss   = GVget("up_capdaily"+AcctSuffix(), 0);
   ...
   if(GlobalVariableCheck("up_daykey"+AcctSuffix()) && (int)GVget("up_daykey"+AcctSuffix(),0)==today)
      g_dayStartEquity = GVget("up_dayeq"+AcctSuffix(), AccountInfoDouble(ACCOUNT_EQUITY));
   else
      SetDayBaseline(today);
   g_dayStart = today;

   EventSetTimer(MathMax(5, InpPollSeconds));
   PrintFormat("Uptick MT5 EA v3.1 started — profile=%s, trading forex + Volatility. Ensure https://legendary-palm-tree-tawny.vercel.app is whitelisted in WebRequest.", ProfileStr());
   Poll();
...
// Stamp the re-entry cooldown when an Uptick position CLOSES (not at entry).
...
void   SetDayBaseline(int day)
  { g_dayStartEquity=AccountInfoDouble(ACCOUNT_EQUITY); GVset("up_daykey"+AcctSuffix(),day); GVset("up_dayeq"+AcctSuffix(),g_dayStartEquity); }
...
void Poll()
  {
   ...
   Comment(StringFormat("Uptick MT5 v3.1 · %s · forex+volatility · %d signals · %s%s%s%s",
...
void ParseCaps(string line)
  {
   ...
   if(p>=0) { double v=StringToDouble(StringSubstr(line, p+12)); if(v>0) { g_maxOpenRisk=v; GVset("up_capmax"+AcctSuffix(), v); } }
   int q = StringFind(line, "corrCap=");
   if(q>=0) { double v=StringToDouble(StringSubstr(line, q+8)); if(v>0) { g_corrCap=v; GVset("up_capcorr"+AcctSuffix(), v); } }
   int dl = StringFind(line, "dailyLoss=");
   if(dl>=0) { double v=StringToDouble(StringSubstr(line, dl+10)); if(v>0) { g_dailyLoss=v; GVset("up_capdaily"+AcctSuffix(), v); } }
...
void OpenBase(Sig &s)
  {
   ...
   bool ok = buy ? g_trade.Buy(lots, sym, price, s.sl, s.tp, "uptick")
                 : g_trade.Sell(lots, sym, price, s.sl, s.tp, "uptick");
   if(ok)
     {
      GVset("up_add_"+sym, 0);                 // reset pyramid counter for this symbol
      BindPlan(s, lots, s.risk);
      int d=(int)SymbolInfoInteger(sym,SYMBOL_DIGITS);
      PrintFormat("Uptick %s %s %.2f lots @ %s SL %s TP %s", s.side, sym, lots,
                  DoubleToString(price,d), DoubleToString(s.sl,d), DoubleToString(s.tp,d));
     }
   else PrintFormat("Uptick order failed %s %s: %d", s.side, sym, g_trade.ResultRetcode());
  }
   ...
   for(int k=0;k<n;k++)
     {
      ...
      int done=(int)GVget("up_add_"+sym, 0);
      ...
      g_trade.SetTypeFillingBySymbol(sym);
      bool ok = buy ? g_trade.Buy(lots, sym, price, sigs[k].sl, sigs[k].tp, "uptick-add")
                    : g_trade.Sell(lots, sym, price, sigs[k].sl, sigs[k].tp, "uptick-add");
      if(ok)
        {
         GVset("up_add_"+sym, done+1);
         BindPlan(sigs[k], lots, apct);
         PrintFormat("Uptick ADD #%d %s %s %.2f lots", done+1, sigs[k].side, sym, lots);
        }
     }
  }
...
void BindPlan(Sig &s, double lots, double riskPct)
  {
   ...
   else PrintFormat("Uptick: opened %s but could not bind plan yet (will retry next poll).", s.sym);
  }
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      ...
      double r=GVget("up_risk_"+(string)tk, 0);
      total+=r;
      if(GVget("up_clu_"+(string)tk, -1)==cid) clus+=r;
     }
...
void TrailStop(ulong tk)
  {
   ...
   double trail=GVget("up_trail_"+(string)tk, 0);
   if(trail<=0) return;
...
void FirePartials(ulong tk)
  {
   if(!PositionSelectByTicket(tk)) return;
   string sym=PositionGetString(POSITION_SYMBOL);
   int np=(int)GVget("up_np_"+(string)tk, 0);
   int pf=(int)GVget("up_pf_"+(string)tk, 0);
   if(pf>=np) return;
   double ov=GVget("up_ov_"+(string)tk, 0);
   long type=PositionGetInteger(POSITION_TYPE);
   double bid=SymbolInfoDouble(sym,SYMBOL_BID), ask=SymbolInfoDouble(sym,SYMBOL_ASK);
   ...
   for(int i=pf;i<np;i++)
     {
      double pp=GVget("up_pp_"+(string)tk+"_"+(string)i, 0);
      double pc=GVget("up_pc_"+(string)tk+"_"+(string)i, 0);
      ...
      if(vol>=vmin && keep>=vmin)                          // keep at least a min-lot runner
        {
         if(g_trade.PositionClosePartial(tk, vol)) { remaining-=vol; GVset("up_pf_"+(string)tk, i+1); }
         else break;                                       // broker rejection → retry next poll
        }
      else GVset("up_pf_"+(string)tk, i+1);                // too small to bank — mark done, don't loop forever
     }
  }
...
void StorePlan(ulong tk, Sig &s, double ov, double riskPct)
  {
   string T=(string)tk;
   GVset("up_trail_"+T, s.trail);
   GVset("up_ov_"+T, ov);
   GVset("up_np_"+T, s.nP);
   GVset("up_pf_"+T, 0);
   GVset("up_risk_"+T, riskPct);
   GVset("up_clu_"+T, ClusterId(s.clu));
   for(int i=0;i<s.nP;i++) { GVset("up_pp_"+T+"_"+(string)i, s.pPrice[i]); GVset("up_pc_"+T+"_"+(string)i, s.pPct[i]); }
  }
   ...
   for(int i=GlobalVariablesTotal()-1;i>=0;i--)
     {
      ...
      if(StringFind(name,"up_trail_")!=0) continue;
...
void DeleteTicketGVs(ulong tk)
  {
   string T=(string)tk;
   GlobalVariableDel("up_trail_"+T); GlobalVariableDel("up_ov_"+T); GlobalVariableDel("up_np_"+T);
   GlobalVariableDel("up_pf_"+T);    GlobalVariableDel("up_risk_"+T); GlobalVariableDel("up_clu_"+T);
   for(int i=0;i<8;i++) { GlobalVariableDel("up_pp_"+T+"_"+(string)i); GlobalVariableDel("up_pc_"+T+"_"+(string)i); }
  }

// Attach a plan to any Uptick position that has none yet (heals a missed bind).
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      ulong tk=PositionGetTicket(i);
      if(tk==0 || PositionGetInteger(POSITION_MAGIC)!=InpMagic) continue;
      if(GlobalVariableCheck("up_trail_"+(string)tk)) continue;
      string sym=PositionGetString(POSITION_SYMBOL);
      long type=PositionGetInteger(POSITION_TYPE);
...
//+------------------------------------------------------------------+
string GVKey(string sym) { return "uptick_last_"+sym; }
   ...
   if(code==-1)
     {
      ...
      if(err==4014 || err==4060) PrintFormat("WebRequest blocked — add https://legendary-palm-tree-tawny.vercel.app in Tools>Options>Expert Advisors.");
